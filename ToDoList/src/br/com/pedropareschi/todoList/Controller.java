...
//        todoListView.setItems(TodoData.getInstance().getTodoItems());
    ...
    public void showNewItemDialogue(){
        ...
        dialog.setHeaderText("Use this dialog to creat a new to-do item");
    ...
    public void editItem(TodoItem oldItem){
        Dialog<ButtonType> dialog = new Dialog<>();
        dialog.initOwner(mainBorderPane.getScene().getWindow());
        dialog.setTitle("Edit New Todo Item");
        dialog.setHeaderText("Use this dialog to edit a to-do item");
        FXMLLoader fxmlLoader = new FXMLLoader();
        fxmlLoader.setLocation(getClass().getResource("todoItemDialogue.fxml"));
        try{
            dialog.getDialogPane().setContent(fxmlLoader.load());
        ...
        DialogController controller = fxmlLoader.getController();
        controller.showPreviousDetails(oldItem);
        dialog.getDialogPane().getButtonTypes().add(ButtonType.OK);
        dialog.getDialogPane().getButtonTypes().add(ButtonType.CANCEL);

        Optional<ButtonType> result = dialog.showAndWait();
        if(result.isPresent() && result.get() == ButtonType.OK) {
            TodoItem newItem = controller.processResults(oldItem);
            todoListView.getSelectionModel().select(newItem);
        }
    }
