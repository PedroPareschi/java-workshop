...
public class SortedArray {
    public static int[] getIntegers(int size){
        ...
        System.out.println("Type " + size + " integer values");
    ...
    public static int[] sortIntegers(int[] array){
        ...
        int greatestNumber, previousGreatestNumber = Integer.MAX_VALUE;
        for(int i = 0; i < sortedArray.length; i++){
            greatestNumber = Integer.MIN_VALUE;
            for(int j = 0; j < array.length; j++){
                if((array[j] > greatestNumber) && (array[j] < previousGreatestNumber)){
                    greatestNumber = array[j];
                }
            }
            sortedArray[i] = greatestNumber;
            previousGreatestNumber = greatestNumber;
        }
        return sortedArray;
    }
